...
class SceneClothHang : public Scene
{
public:
	SceneClothHang() { name = "Cloth / Hang"; }
...
class SceneClothHang_Horizontal : public Scene
{
public:
	SceneClothHang_Horizontal() { name = "Cloth / Hang_Horizontal"; }

	void PopulateActors(GameInstance* game)  override
	{
		SpawnCameraAndLight(game);
		SpawnInfinitePlane(game);
		ModifyParameter(&Global::simParams.gravity, glm::vec3(0,0,0));
		
		//ModifyParameter(&Global::simParams.gravity, 0.0f);
		int clothResolution = 64;

		ModifyParameter(&Global::simParams.offset_index, (clothResolution/2) * (clothResolution + 1) + clothResolution/2);
		ModifyParameter(&Global::simParams.offset, glm::vec3(0,0,1));
		//vector<int> x_indexlist = { clothResolution / 2 };
		//vector<int> y_indexlist = { clothResolution / 2 };
		//vector<glm::vec3> offset_list = { glm::vec3{0,0,clothResolution / 2} };
		//auto cloth = SpawnCloth_Release(game, x_indexlist,y_indexlist,offset_list, clothResolution, 2);
		auto cloth = SpawnCloth(game, clothResolution, 2);
		cloth->Initialize(glm::vec3(0.0f, 1.5f, 1.0f), glm::vec3(1.0), glm::vec3(90, 0, 0));
#ifdef SOLVER_CPU
		auto clothObj = cloth->GetComponent<VtClothObject>();
...
int main()
{
	...
	vector<shared_ptr<Scene>> scenes = {
		...
		make_shared<SceneClothHang>(),
		make_shared<SceneClothHang_Horizontal>(),
