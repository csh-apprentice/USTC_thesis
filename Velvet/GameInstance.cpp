...
void GameInstance::MainLoop()
{
	...
	while (!glfwWindowShouldClose(m_window) && !pendingReset)
	{
		...
		if (Timer::physicsFrameCount() == 11)
		{
			saveImage("./scene_example/hang_cheby9.png",m_window);
		}
		/*
		if (Timer::physicsFrameCount() == 41)
		{
			saveImage("./fig/PBD_41.png", m_window);
		}
		if (Timer::physicsFrameCount() == 81)
		{
			saveImage("./fig/PBD_81.png", m_window);

		}
		if (Timer::physicsFrameCount() == 161)
		{
			saveImage("./fig/PBD_161.png", m_window);
