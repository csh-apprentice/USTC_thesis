...
struct VtSimParams
{
	...
	float relaxationFactor			HOST_INIT(0.3f);					//!< Control the convergence rate of the parallel solver, default: 1, values greater than 1 may lead to instability
	...
	// Jacobi
	float under_relax_coeff          HOST_INIT(0.9f);

	//initial velocities
	glm::vec3 init_v			     HOST_INIT(glm::vec3(0, 0, 0));   //initial constant default seeting for all particels

	//initial offset
	//vector<int> offset_index={1,2,4};      //intial offset index
	int offset_index				HOST_INIT(0);
	glm::vec3 offset				HOST_INIT(glm::vec3(0, 0, 0));
	//vector<glm::vec3> offset         HOST_INIT(vector<glm::vec3>(glm::vec3(0, 0, 0)));       //initial offset
	...
	void OnGUI()
	{
		...
		IMGUI_LEFT_LABEL(ImGui::SliderInt, "Num Iterations", &numIterations, 1, 60);
		...
		ImGui::Separator();
		IMGUI_LEFT_LABEL(ImGui::SliderFloat, "Jacobi", &under_relax_coeff, 0.6, 0.9);
