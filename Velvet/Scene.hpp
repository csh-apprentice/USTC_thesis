...
namespace Velvet
{
	...
	protected:
		...
		shared_ptr<Mesh>  GenerateClothMesh_Release(int resolution,vector<int> x_indexlist,vector<int> y_indexlist,vector<glm::vec3> offset_list)
		{
			vector<glm::vec3> vertices;
			vector<glm::vec3> normals;
			vector<glm::vec2> uvs;
			...
			{
				vertices[VertexIndexAt(x_indexlist[index], y_indexlist[index])] += clothSize * glm::vec3(offset_list[index].x / (float)resolution, offset_list[index].y / (float)resolution, offset_list[index].z / (float)resolution);//offset_list[index];
			}
		...
		shared_ptr<Actor> SpawnCloth_Release(GameInstance* game, vector<int> x_indexlist, vector<int> y_indexlist, vector<glm::vec3> offset_list, int resolution = 16, int textureFile = 1, shared_ptr<VtClothSolverGPU> solver = nullptr)
		{
			auto cloth = game->CreateActor("Cloth Generated");
			...
			auto mesh = GenerateClothMesh_Release(resolution, x_indexlist,y_indexlist,offset_list);
			//auto mesh = GenerateClothMeshIrregular(resolution);
